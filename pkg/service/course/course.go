...
func (s *service) listCourses(ctx context.Context, opts ...store.CourseListOption) (*acourse.CoursesResponse, error) {
	...
	for i, course := range courses {
		c, err := s.store.EnrollCourseCount(ctx, course.GetID())
		if err != nil {
			return nil, err
		}
...
func (s *service) GetCourse(ctx context.Context, req *acourse.CourseIDRequest) (*acourse.CourseResponse, error) {
	...
	enroll, err := s.store.EnrollFind(ctx, userID, course.GetID())
	if err != nil {
		return nil, err
	}
	if enroll != nil || course.Owner == userID {
		var attend *model.Attend
		attend, err = s.store.AttendFind(ctx, userID, course.GetID())
		if err != nil {
			return nil, err
		}
	...
	payment, err := s.store.PaymentFind(ctx, userID, course.GetID(), model.PaymentStatusWaiting)
	if err != nil {
		return nil, err
	}
...
func (s *service) AttendCourse(ctx context.Context, req *acourse.CourseIDRequest) (*acourse.Empty, error) {
	...
	err = s.store.AttendSave(ctx, &model.Attend{UserID: userID, CourseID: course.GetID()})
	if err != nil {
		return nil, err
	}
